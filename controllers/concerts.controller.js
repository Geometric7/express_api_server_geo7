...
exports.getPerformer = async (req, res) => {
  try {
    ...
    const getPerformer = await Concert.find({
      performer: performer
    });
    if (!getPerformer) res.status(404).json({
      message: 'Not found'
    });
    else {
...
exports.getGenre = async (req, res) => {
  try {
    ...
    const genre = await Concert.find({
      genre: dep
    });
...
exports.getPrice = async (req, res) => {
  try {
    ...
    if (!concert) res.status(404).json({
      message: 'Not found'
    });
    else {
      res.json(concert);
